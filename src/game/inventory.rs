...
impl SlotCount {
	...
	pub const HOTBAR: u8 = 5;
	pub const INV_ROW: u8 = 3;
	pub const INV_COL: u8 = 7;
...
impl InventoryLayout {
    pub fn calculate_for_player(inv_state: InvState) -> Self {
        ...
        layout.inv_area = AreaLayout::new(
            SlotCount::INV_ROW, SlotCount::INV_COL, 
...
impl UIManager {
    ...
    fn add_player_buttons(&mut self, layout: &InventoryLayout) {
        let panel_left = layout.panel_position.0;
        let panel_bottom = layout.panel_position.1;
        let button_width = 0.35;
        let button_height = SLOT;
        let button_spacing = 0.02;
        
        // Position buttons at bottom-left of panel
        let memory_x = panel_left - button_width - button_spacing;
        let memory_y = panel_bottom;
        
        let memory_button = UIElement::button(self.next_id(), "Memory")
            .with_position(memory_x, memory_y)
