...
pub async fn load_string(file_name: &str) -> io::Result<String> {
...
pub async fn load_binary(file_name: &str) -> io::Result<Vec<u8>> {
    let path = Path::new(env!("OUT_DIR"))
        .join("res")
        .join(file_name);
    std::fs::read(path)
...
) -> Result<model::Model, Box<dyn std::error::Error + Send + Sync>> {
