...
mod tests {
	...
	fn border_block_detection() {
		...
		let max = Chunk::SIZE_I;

		// Test edge cases
		assert!(chunk.is_border_block(IVec3::new(0, 5, 5)));
		assert!(chunk.is_border_block(IVec3::new(max-1, 5, 5)));
		assert!(chunk.is_border_block(IVec3::new(5, 0, 5)));
		assert!(chunk.is_border_block(IVec3::new(5, max-1, 5)));
		assert!(chunk.is_border_block(IVec3::new(5, 5, 0)));
		assert!(chunk.is_border_block(IVec3::new(5, 5, max-1)));

		// Test non-border cases
		assert!(!chunk.is_border_block(IVec3::new(1, 1, 1)));
		assert!(!chunk.is_border_block(IVec3::new(max-2, max-2, max-2)));
	}
	...
	fn palette_compaction() {
		...
		for i in 0..10 {
			let pos = LocalPos::new(i % 16, (i / 16) % 16, 0);
	...
	fn storage_transitions() {
		...
		assert!(matches!(chunk.storage(), BlockStorage::Uniform { .. }));
		assert_eq!(chunk.get_block(LocalPos::ZERO.into()), block(1));
		assert_eq!(chunk.get_block(LocalPos::CORNER.into()), block(1));

		// 2. Setting one non-default block should transition to Compact storage
		let pos1 = LocalPos::new(1, 1, 1);
		let block1 = block(2);
...
*/
	...
	fn memory_usage_reporting() {
		...
		assert_eq!(size, Chunk::VOLUME * std::mem::size_of::<Block>());
	}
