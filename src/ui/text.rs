...
use crate::{get_bytes, ui::render::UIRenderer};
...
impl UIRenderer {
	...
	pub fn render_text_to_texture(&self, device: &wgpu::Device, queue: &wgpu::Queue, text: &str, element_size: Vec2, color: Color, mode: TruncateMode, align: AlignMode) -> wgpu::Texture {
		...
		let (final_text, final_scale) = if text_width > target_width_px as f32 * 0.95 {
			self.truncate_text(text, scale, target_width_px as f32, &mode)
		} else {
		...
		// Calculate the texture width based on the mode
		let (width, text_start_x) = {
			let texture_width = target_width_px.max((final_width.ceil() as u32 + padding * 2).max(1));
			let start_x = self.calculate_x(&final_text, final_scale, texture_width as f32, align);
		...
		let adjusted_glyphs: Vec<_> = self.font.layout(&final_text, final_scale, point(text_start_x, v_metrics.ascent + padding as f32)).collect();
	...
	fn calculate_x(&self, text: &str, scale: Scale, texture_width: f32, align: AlignMode) -> f32 {
		...
		match align {
			AlignMode::Lm => 0.0, // Align start to left edge
			AlignMode::Cm => (texture_width - text_width) / 2.0, // Center the text
			AlignMode::Rm => texture_width - text_width, // Align end to right edge
			_ => 0.0, // Shouldn't happen as this is only called for push modes
		}
	}
		...
		while left_idx <= right_idx {
			...
			if left_idx <= right_idx {
				...
				if left_width + advance <= available_width / 2.0 {
					...
					if right_width + right_advance > available_width - left_width { break; }
			...
			if left_idx <= right_idx {
				...
				if right_width + advance <= available_width / 2.0 {
					...
					if left_width + left_advance > available_width - right_width { break; }
	...
	fn shrink_to_fit(&self, text: &str, scale: Scale, max_width: f32) -> (String, Scale) {
		...
		for _ in 0..8 { // 8 iterations should be precise enough
	...
	fn grow_to_fit(&self, text: &str, scale: Scale, max_width: f32) -> (String, Scale) {
		...
		let mut max_scale = scale.x * 20.0; // Limit growth to 20x original size
		let mut optimal_scale = scale.x;
